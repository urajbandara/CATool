		...
		for t in range(0,n):
			...
			for i in xrange( 0, g.rows):
				for j in xrange( 0, g.columns):
					...
					#print("i-" + str(i) + "  j-"  + str(opinionsum))
					if opinionsum < 0.2:
						g.m[i][j] = 0
                                        elif opinionsum >= 0.2 and opinionsum < 0.4:
                                                g.m[i][j] = 0.3
                                        elif opinionsum >= 0.4 and opinionsum < 0.6:
                                                g.m[i][j] = 0.5
                                        elif opinionsum >= 0.6 and opinionsum < 0.8:
                                                g.m[i][j] = 0.70
                                        elif opinionsum >= 0.8:
                                                g.m[i][j] = 1
...
def display_state():
	...
	for i in range(0,sz):
		for j in range(0,sz):                   
			if state[i][j] < 0.2 :
				glColor3f(0,1.0,0)
			elif state[i][j] >= 0.2 and state[i][j] < 0.4:
				glColor3f(0,1.0,1.0)
			elif state[i][j] >= 0.4 and state[i][j] < 0.6:
				glColor3f(1.0,0,1.0)
			elif state[i][j] >= 0.6 and state[i][j] < 0.8:
				glColor3f(1.0,1.0,0)
			elif state[i][j] >= 0.8:
				glColor3f(1.0,0,0)                      
